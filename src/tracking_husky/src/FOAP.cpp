...
/*Se asigna la misma altura de los agentes al obstaculo*/
   ...
   //Obstacle detected
    ...
    else
    {
        ...
        Eigen::VectorXd q_diff = q_i - temp;
        arg=pow(q_diff[0],2)+pow(q_diff[1],2);
        r_k=sqrt(arg);
...
Eigen::MatrixXd FOAP::pseudoinverse(Eigen::MatrixXd J, double lambda){
    ...
    if(rows==1){ // case when J = 1xn
        J_inv = J_T * pow(JJ_T(0,0) - pow(lambda,2), -1);
    }
    else{ // case when J = mxn
        Eigen::MatrixXd I;
...
Eigen::VectorXd FOAP::centroidError(Eigen::MatrixXd const qz, Eigen::VectorXd const qz_centroid){
    int rows, cols;
    rows = qz.rows(); cols = qz.cols(); // dim, n_agents
    Eigen::VectorXd e;
...
Eigen::VectorXd FOAP::alfa_vel(Eigen::MatrixXd const E, Eigen::MatrixXd const vel){
    int rows, dim;
    rows = E.rows(); // number of agents
    dim = vel.size()/rows; // dimension of the system
    Eigen::MatrixXd I = Eigen::MatrixXd::Identity(dim,dim);
    Eigen::VectorXd qz_vector = matrix2vector(vel);
    Eigen::VectorXd ez = Eigen::kroneckerProduct(I,E).eval()*qz_vector;
...
/*
Asymptotic consensus (only for 2 dimensions)
k1,k2 gains
e_i consensus error
*/
Eigen::VectorXd FOAP::gains(Eigen::VectorXd e_i,double k1, double k2,int n_robots)
{    Eigen::VectorXd u=Eigen::VectorXd::Zero(n_robots*2);
    for(int i=0;i<n_robots;i++)
    {
        u(i)=k1*e_i(i);
        u(i+n_robots)=k2*e_i(i+n_robots);
    }
    return u;
}
...
q = q_1 + q_1|2
*/
Eigen::VectorXd FOAP::obstacleAvoidance(Eigen::MatrixXd const q, Eigen::MatrixXd const q_ev, Eigen::VectorXd const ratios,
                                Eigen::VectorXd const p_ia, Eigen::MatrixXd const J_2, TTF &ht,
                                double t, double d, double dt, int i_agent,float nearestDist){

    int rows, cols, ratios_size;
    double lambda_error = 1.5;
    rows = q.rows(); cols = q.cols(); ratios_size = ratios.size(); // rows = pose 3D, cols = number of agents/robots. ratios_size = number of ratios
    ...
    double eo;
    int type=1;
    Eigen::VectorXd eo_nearest=q_ev.col(i_agent); // nearest obstacle or agent

    //int Idx // to distinguish between agent and obstacle
    
    eo = double(nearestDist)-ratios(1);//obstacleError(nearest, distances, ratios, eo_nearest);
    //type = obstacleType(nearest, distances, ratios);
    //std::cout<<"Obstacle error "<<i_agent<<": "<<eo<<std::endl;
  /* if (type==1)
   {
    ...
    //if(eo>0){
    if(eo>0 && ht.t0_o(i_agent)==0 && ht.xi(i_agent)==0){
        q_c = p_ia;

        return q_c;
    }
    else{ // the soft transition are activated for the obstacle avoidance
        
        /*q_c.resize(2);
        q_c(0)=0;
        q_c(1)=0;
        return q_c;*/
        xi(ht, eo, t, d, dt, i_agent);
    ...
    //std::cout<<"h(t): "<<ht.xi(i_agent)<<std::endl;
    // ê1_t = h(t)ê1 + (1- h(t))J2J1⁺ê2
    Eigen::MatrixXd Jp_2 = pseudoinverse(J_2);
    Eigen::MatrixXd Jp_o = pseudoinverse(J_o);
    double e1_t = -lambda_error*ht.xi(i_agent)*eo +((1.0-ht.xi(i_agent))*(J_o*Jp_2*p_ia)(0,0));
...
Eigen::VectorXd FOAP::convexCombination_noht(Eigen::MatrixXd const q, Eigen::MatrixXd const qobs, Eigen::VectorXd const ratios,
                                Eigen::VectorXd const p_ia, Eigen::MatrixXd const J_2, TTF &ht,
                                double t, double d, double dt, int i_agent){

    int rows, cols, ratios_size;
    double lambda_error = .5;
    rows = q.rows(); cols = q.cols(); ratios_size = ratios.size(); // rows = pose 3D, cols = number of agents/robots. ratios_size = number of ratios
...
TBG gain function 
*/
Eigen::VectorXd FOAP::TTFgain(double d, double t0, double tf, double a0, double t){
...
Eigen::VectorXd FOAP::TTFgain_consensus(double tf,double t,double kf){
...
*/
void FOAP::xi(TTF &ht, double eo, double t, double d, double dt, int i_agent){    
    Eigen::VectorXd ansTTF; // TBGgain_cos return [a, Xi, Xip]
    // negative error and the times are equal to zero, this indicates that the obstacle was detected for the first time
    if(ht.t0_o(i_agent)==0 && ht.tf_o(i_agent)== 0 && eo<0){
        // The start and end times of the function are assigned
        ht.t0_o(i_agent) = t;
        ht.tf_o(i_agent) = t + ht.t_up;
        ansTTF = TTFgain(d, ht.t0_o(i_agent), ht.tf_o(i_agent), ht.a_0, t);
        ht.a_gain(i_agent) = ansTTF(0); ht.xi(i_agent) = ansTTF(1); ht.xip(i_agent) = ansTTF(2);
    ...
    else if(ht.t0_o(i_agent)!=0 && ht.tf_o(i_agent)!=0){
        // If the time is within the limit of the final time it continues to grow smoothly
        if(t<(ht.tf_o(i_agent)+ht.t_a)){
            ansTTF = TTFgain(d, ht.t0_o(i_agent), ht.tf_o(i_agent), ht.a_0, t);
            ht.a_gain(i_agent) = ansTTF(0); ht.xi(i_agent) = ansTTF(1); ht.xip(i_agent) = ansTTF(2);
        } // If the time is no longer within the final time and activation time, the function (xi1) will gradually decrease from 1 to 0
        else if(t<(ht.tf_o(i_agent)+ht.t_a+ht.t_down+dt)){
            ansTTF = TTFgain(d, ht.tf_o(i_agent)+ht.t_a, ht.tf_o(i_agent)+ht.t_a+ht.t_down+dt,ht.a_0, t);
            ht.xi(i_agent) = 1.0 - ansTTF(1);
            ...
            if(eo<0){
                ht.tf_o(i_agent) = 2*t-ht.t_a-ht.t_up-ht.t0_o(i_agent);
                ht.t0_o(i_agent) = ht.tf_o(i_agent)-ht.t_up;
            }
        }
        else{ // If it does not enter the conditions, the initial values ​​are assigned until another obstacle is found
            ht.xi(i_agent) = 0;
            ht.tf_o(i_agent) = 0;
            ht.t0_o(i_agent) = 0;
        }
...
*/
void TTF::init_ht(const int n_robots, const int dim, Eigen::VectorXd const params){
    if(params.size()<5){
        std::cout << "Default TTF inicialization" << std::endl;
...
*/
//argumento=pow(1/a_axis,2)*pow(q_current[0]-v[0],2)+pow(1/b_axis,2)*pow(q_current[1]-v[1],2);
    argumento=pow(q_current[0]-v[0],2)+pow(q_current[1]-v[1],2);
if (argumento<1){
...
double FOAP::nearestObstacle(Eigen::VectorXd const q_i, Eigen::MatrixXd const qobs, Eigen::VectorXd &nearest){
    ...
    if(cols == 0){ // no fixed obstacles, we crete an imaginary obstacle
